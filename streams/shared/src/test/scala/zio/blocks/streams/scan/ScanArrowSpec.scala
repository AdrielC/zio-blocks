...
/**
 * Arrow-class parity tests. Mirrors fs2's `Scan` surface: `first`, `second`,
 * `***`, `lens`, `semilens`, `semipass`, `left`, `right`, `+++` (choice), `|||`
 * (choose), `step`.
 */
  ...
  def spec: Spec[TestEnvironment, Any] = suite("Scan arrow parity")(
    ...
    suite("lens / semilens / semipass")(
      ...
      test("semilens may short-circuit some inputs by returning Left") {
        ...
        val s = Scan
          .runningFold[Int, Int](0)(_ + _)
          .semilens[Int, Int](i => if (i < 0) Left(-1) else Right(i), (_, sum) => sum)
